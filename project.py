...
APPLICATION_NAME = "Restaurant Menu Web App"
...
def deleteRestaurant(restaurant_id):
    ...
    if restaurantToDelete.user != login_session['user_id']:
        return """<script>
        function notAuthorizedAlarm(){
            alert('You are not authorized to delete this restaurant');
        }
        </script>
        <body onload='notAuthorizedAlarm()'>
        """
    if request.method == 'POST':
...
def newMenuItem(restaurant_id):
    ...
    if request.method == 'POST':
        newItem = MenuItem(
            ...
            # I think it supposed to be login_session['user_id]
